...
trait RevenueCatIntegrationTesting
{
    ...
    public function makeProductWithStoreIds(?bool $subscription = false, ?bool $subscriptionWithTrial = false): array
    {
        $appStoreProductId = $this->faker->uuid;
        $playStoreProductId = $this->faker->uuid;
        $product = Product::factory()
            ->when($subscription && !$subscriptionWithTrial, fn($factory) => $factory->subscriptionWithoutTrial()->state(['subscription_period' => PeriodEnum::DAILY, 'subscription_duration' => 3]))
            ->when($subscription && $subscriptionWithTrial, fn($factory) => $factory->subscriptionWithTrial()->state(['subscription_period' => PeriodEnum::DAILY, 'subscription_duration' => 3]))
            ->state([
                'fields' => [
                    'in_app_purchase_ids' => [
                        'revenuecat' => [
    ...
    public function makeWebhookSubscriptionPayloadWith(int $userId, string $eventType, string $productId, string $periodType, string $store, Carbon $expiresAt, array $data = []): array
    {
        $payload = [
            'api_version' => '1.0',
            'event' => [
    ...
    protected function makeRevenueCatSubscriptionResponse(string $productId, Carbon $expiresDate, ?string $periodType = 'normal'): array
    {
        return [
            'subscriber' => [
                'subscriptions' => [
                    $productId => [
                        'expires_date' => $expiresDate->toDateTime()->getTimestamp(),
                        'period_type' => $periodType
                    ]
                ]
            ]
